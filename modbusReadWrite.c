...
//***************** Pascal ************************
//user input = ./modbusReadWrite RegisterNumber RegisterValue ex. ./modbusReadWrite 8 255 put register 8 to 255
//user input change: ./modbusReadWrite 255 255 255 255 255 255 255 255 (all 8 output values to set from 0 to 255)
//Notes: registers 0 to 7 => inputs 1 to 8 AND registers 8 to 15 => outputs 1 to 8
...
int main( int argc, char *argv[] )
{
	...
	// write OUTPUTS values to modbus only if the new values are different from old values
	//(writeHoldingRegister(slaveaddr, regaddr, value, 1000)
	for (i=0; i<8; i++) {
		if (newOutputs[i] != oldOutputs[i]) {
			if (writeHoldingRegister(1, i+8, newOutputs[i], 1000) < 0)
				{
					printf("Failed to read register from mobus device !");
					// Close Port
					closeModbus();
	...
	// read INPUTS values from modbus (registers 0 to 7)
	//(readHoldingRegister(slaveaddr, regaddr, &value, 2000)
	if (readHoldingRegister(1, 0, &inputs, 2000) < 0)
    {	
		printf("Failed to read register from mobus device !");				
		// Close Port
		closeModbus();	
	...
	//print "input A/D values" as a json string
	printf("%s","{");
	for (i=0; i<8; i++) {
		printf("%c",'"');
		printf("%s",keys[i]);
